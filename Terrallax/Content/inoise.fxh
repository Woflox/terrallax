...
float perm(float x)
{
	return tex1Dlod(permSampler, float4(x, 0, 0, 0)).x;
}
...
//2D
float gradperm(float x, float2 p)
{
...
{
	float2 P = fmod(floor(p), 256.0);	// FIND UNIT CUBE THAT CONTAINS POINT
  	p -= floor(p);                      // FIND RELATIVE X,Y,Z OF POINT IN CUBE.
	float2 f = fade(p);                 // COMPUTE FADE CURVES FOR EACH OF X,Y,Z.
...
float mf(float2 p, int octaves, float spectral_exp = 0.75, float lacunarity = 2.0, float offset = 1.5, float threshold = 0.9)
{
	float sum = 0;
	float freq = 1.0; 
	float amp = 0;
